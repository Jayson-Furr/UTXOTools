...
/// <summary>
/// Command to locate all P2SH scriptPubKeys that could potentially be P2SH-P2WPKH 
/// (P2WPKH wrapped in P2SH) and export them to a binary file.
/// </summary>
/// <remarks>
/// <para>
/// P2SH-P2WPKH outputs are indistinguishable from other P2SH outputs at the scriptPubKey level.
/// The difference is only revealed when spending, as the redeem script would be:
/// OP_0 0x14 &lt;20-byte witness pubkey hash&gt; (22 bytes total)
/// </para>
/// <para>
/// Since the UTXO set only contains scriptPubKeys (not redeem scripts), this command extracts
/// all P2SH outputs. The output can be used to match against a known set of P2SH-P2WPKH 
/// script hashes if available from other sources.
/// </para>
/// </remarks>
internal static class P2shP2wpkhCommand
{
    /// <summary>
    /// Binary file format:
    /// - Header (9 bytes):
    ///   - 4 bytes: Magic "SHWP" (0x53 0x48 0x57 0x50)
    ...
    public static int Execute(string[] args)
    {
        if (args.Length < 2)
        {
            Console.Error.WriteLine("Usage: UTXOTools.Demonstration p2sh-p2wpkh <input-file> <output-file> [--include-amount] [--verbose]");
            ...
            Console.Error.WriteLine();
            Console.Error.WriteLine("Note: P2SH-P2WPKH outputs cannot be distinguished from other P2SH outputs");
            Console.Error.WriteLine("      at the scriptPubKey level. This command extracts all P2SH outputs.");
            Console.Error.WriteLine("      The redeem script (which identifies P2SH-P2WPKH) is only revealed");
        ...
        Console.WriteLine($"Scanning for P2SH scripts (potential P2SH-P2WPKH) in {inputPath}...");
        Console.WriteLine();
        Console.WriteLine("Note: P2SH-P2WPKH cannot be distinguished from other P2SH at scriptPubKey level.");
        ...
        // Collect P2SH entries (potential P2SH-P2WPKH)
        var entries = new List<P2shP2wpkhEntry>();
        ...
        foreach (var tx in reader.ReadTransactions())
        {
            ...
            foreach (var output in tx.Outputs)
            {
                ...
                {
                    entries.Add(new P2shP2wpkhEntry
                    {
        ...
        Console.WriteLine();
        Console.WriteLine();
        Console.WriteLine($"=== P2SH-P2WPKH Scan Results ===");
        Console.WriteLine($"Total UTXOs scanned:      {entryCount:N0}");
        Console.WriteLine($"P2SH entries found:       {entries.Count:N0}");
        Console.WriteLine($"(Potential P2SH-P2WPKH - requires external verification)");
        Console.WriteLine();
    ...
    /// 
    /// For P2SH-P2WPKH, the hash is HASH160 of the redeem script:
    /// OP_0 (0x00) 0x14 (20 bytes) &lt;20-byte witness pubkey hash&gt;
    /// 
    ...
    /// <summary>
    /// Writes the P2SH-P2WPKH entries to a binary file.
    /// </summary>
    /// <remarks>
    /// Binary format:
    /// - Header (9 bytes):
    ...
    /// </remarks>
    private static void WriteBinaryOutput(string outputPath, List<P2shP2wpkhEntry> entries, bool includeAmount)
    {
        using var stream = File.Create(outputPath);
        using var writer = new BinaryWriter(stream);

        // Write magic bytes "SHWP" (SH-WPkh)
        writer.Write((byte)'S');
        writer.Write((byte)'H');
        writer.Write((byte)'W');
        writer.Write((byte)'P');
    ...
    /// <summary>
    /// Represents a P2SH-P2WPKH entry with its script hash and metadata.
    /// </summary>
    private sealed class P2shP2wpkhEntry
    {
